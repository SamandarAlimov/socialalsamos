...
async function purgeSoftDeletedAccount(admin: ReturnType<typeof createClient>, userId: string) {
  ...
  // If no retained business/financial row restricts profile deletion, removing
  // the profile cascades through the user's social data. Otherwise keep only a
  // non-identifying tombstone profile so retained records do not expose PII.
...
serve(async (req) => {
  ...
  // First try a true hard delete. The profiles row references auth.users with
  // ON DELETE CASCADE, so normal social/profile data is removed automatically.
  ...
  // Some financial/business history intentionally uses restrictive foreign keys.
  // In that case disable the Auth identity permanently, then remove/anonymize the
  // user-facing profile and personal settings without destroying retained records.
