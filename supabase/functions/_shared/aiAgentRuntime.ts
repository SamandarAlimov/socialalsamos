...
function sysPrompt(opts: {
  ...
  const fileRules = opts.toolNames.includes("run_code")
    ? `FILE DELIVERABLES
- If the user asks for an Excel/XLSX, Word/DOCX, PowerPoint/PPTX, PDF, CSV, Markdown, text, or JSON file, create the ACTUAL requested file with run_code instead of merely printing a table, Markdown, code, or a fake download link.
- Prefer Python for office/PDF deliverables. Save each final file under /workspace/outputs/<clear-filename> so it is collected and shown as a Presented file.
- The Python sandbox includes openpyxl, python-docx, python-pptx and reportlab. The workspace is temporary; only files in /workspace/outputs are collected.
- Downloadable binary files are Files, not Artifacts. Do not claim a file exists unless the tool result contains presented_files/files metadata.
- For spreadsheets, put formulas in cells when appropriate and keep source assumptions/labels readable. For DOCX/PPTX/PDF, create a polished usable document, not a placeholder.
  ...
  for (const event of eventRows ?? []) {
    ...
    if (Array.isArray(data?.files)) {
      for (const file of data.files) {
        ...
        if (attachments.some((item: any) =>
          (storagePath && item.storagePath === storagePath) || (!storagePath && url && item.url === url)
        )) continue;
        attachments.push({
          url,
          name,
