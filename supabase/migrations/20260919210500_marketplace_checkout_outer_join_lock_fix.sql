-- Fix marketplace checkout regression introduced by the restaurant-aware checkout.
-- PostgreSQL cannot lock the nullable side of a LEFT JOIN. Variants are locked
-- first in a separate query, then base product rows are locked during validation.
...
begin
  ...
  -- Lock selected variant rows independently. Locking pv from the LEFT JOIN
  -- below would raise "FOR UPDATE cannot be applied to the nullable side".
  perform 1
  ...
  -- Validate every cart line and lock only the mandatory product side.
  for v_item in
