...
create policy "RBAC admins can update verification state"
  ...
  for update
  to authenticated
  using (public.admin_control_authorized('verification.review'))
  with check (public.admin_control_authorized('verification.review'));
...
create policy "RBAC admins can send verification notifications"
  ...
  to authenticated
  with check (
    public.admin_control_authorized('verification.review')
...
create policy "RBAC admins can manage legacy roles"
  ...
  for all
  to authenticated
  using (public.admin_control_authorized('admin.roles.manage'))
  with check (public.admin_control_authorized('admin.roles.manage'));
...
create policy "RBAC admins can view all posts"
  ...
  for select
  to authenticated
  using (public.admin_control_authorized('content.view'));
...
create policy "RBAC admins can moderate posts"
  ...
  for delete
  to authenticated
  using (public.admin_control_authorized('content.moderate'));
