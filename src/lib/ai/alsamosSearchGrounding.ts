...
function stripInternalContext(value: string): string {
  return value
    ...
    .replace(/\[FORWARDED POST\][\s\S]*$/gi, ' ')
    .replace(/\[(?:image|video|document|audio|file)\]\s+[^\n]+:\s*https?:\/\/\S+/gi, ' ')
...
function simplifySearchCommand(value: string): string {
  const cleaned = value
    ...
    .replace(/\b(?:qil|qiling|ber|bering|top|topib\s+ber|haqida|bo['’]?yicha)\b/gi, ' ')
    .replace(/[<>]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function mergePayloads(payloads: SearchPayload[]): SearchPayload {
  ...
  const firstUseful = payloads.find((payload) => (payload.results?.length ?? 0) > 0) ?? payloads[0] ?? {};
  return {
    ...firstUseful,
    results,
    summary: payloads.map((payload) => payload.summary).filter(Boolean).join('\n') || firstUseful.summary || null,
...
export async function withAlsamosSearchGrounding<T extends GroundableAgentOptions>(options: T): Promise<T> {
  ...
  try {
    ...
    const primary = await fetchLiveSearch(query, 'all', 12, options.signal);
    payloads.push(primary);

    if ((primary.results?.length ?? 0) === 0 && query !== originalQuery) {
    ...
    if (IMAGE_INTENT.test(originalQuery)) {
      try {
        ...
        // Web results are still useful when an image provider is temporarily unavailable.
      }
    }
