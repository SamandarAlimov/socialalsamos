...
/** Read the Y translation Safari/Chrome expose after compositing transforms. */
...
function sheetVisualTop(sheet: HTMLElement) {
  ...
  // Downward compact -> dismiss uses transform while the logical `top` remains
  // pinned to the compact detent. Different mobile browsers expose that motion
  // through different channels at slightly different times. Take the furthest
  // valid visual edge instead of trusting only one source. This is especially
  // important on iOS Safari where getBoundingClientRect can trail the compositor
  // by a frame while the inline transform is already current.
  return Math.max(
    0,
    logicalTop,
...
/**
 * Keep the visible Reel tied to the comments sheet's actual visual top edge.
 * We intentionally apply geometry on every RAF while the mobile sheet exists,
 * not only while React reports a drag. Upward motion changes `top`; downward
 * compact dismissal changes `transform`. Treating both as one visual position
 * makes the Reel and sheet move as a single surface in both directions.
 */
