...
export default function AdminModerationHubPage() {
  ...
  const tools = [
    ...
    { title: 'Trust & Safety', description: 'Universal report queue, case investigation, evidence, enforcement va appeal lifecycle.', icon: ShieldCheck, path: '/admin/trust-safety', enabled: hasPermission('admin.trust_safety.view') || hasPermission('reports.view') || hasPermission('reports.review'), badge: 'Cases' },
    { title: 'Operations', description: 'Incident response, security pulse, feature rollout va admin auditni boshqarish.', icon: ShieldAlert, path: '/admin/operations', enabled: hasPermission('admin.operations.view') || hasPermission('admin.system.view'), badge: 'Ops' },
    { title: 'System Control', description: 'Background queues, database health, audit diff va session/device security.', icon: ServerCog, path: '/admin/system', enabled: hasPermission('admin.system.view') || hasPermission('admin.operations.view'), badge: 'Health' },
    { title: 'Admin Inbox', description: 'Role-targeted Trust & Safety, security va operations alertlarini ko‘rish.', icon: Bell, path: '/admin/notifications', enabled: true, badge: 'Inbox' },
    ...
    { title: 'Admin huquqlari', description: 'Operatorlar va kirish rollarini boshqarish.', icon: ShieldCheck, path: '/admin/access', enabled: hasPermission('admin.roles.view') || hasPermission('admin.roles.manage') },
