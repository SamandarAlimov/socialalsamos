        ...
        <DrawerContent
          ...
          onTouchStartCapture={(event) => {
            if (
              ...
              activeSnapPoint >= MOBILE_AUDIENCE_SNAP_EXPANDED - 0.01 ||
          ...
          onTouchMoveCapture={(event) => {
            ...
            const touch = event.touches.item(0);
            if (!gesture || !touch || gesture.expanded) return;
...
function CompactAudienceTab({
  ...
  return (
    ...
    >
      <span className={cn('transition-colors', active ? 'text-foreground' : 'text-muted-foreground')}>{icon}</span>
      <span>{label}</span>
      <span
        className={cn(
      ...
      <span
        className={cn(
          ...
          active ? 'scale-x-100 bg-foreground/85 opacity-100' : 'scale-x-50 bg-transparent opacity-0',
        )}
      />
    </button>
  );
