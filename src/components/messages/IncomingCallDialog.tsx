...
export function IncomingCallDialog({
  ...
  const playRingtone = useCallback(() => {
    ...
    if ('vibrate' in navigator && (!navigator.userActivation || navigator.userActivation.hasBeenActive)) {
      const vibrate = () => {
        ...
        try { navigator.vibrate([400, 200, 400, 1600]); } catch {}
      };
      vibrate();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleDecline()}>
      <DialogContent
        className="w-[calc(100vw-1.5rem)] max-w-sm overflow-hidden rounded-3xl border-white/10 bg-neutral-900/95 p-0 text-white backdrop-blur-2xl sm:max-w-md"
        aria-describedby="incoming-call-description"
      >
        <DialogTitle className="sr-only">
          {callType === 'video' ? 'Kiruvchi video qo‘ng‘iroq' : 'Kiruvchi qo‘ng‘iroq'}
        </DialogTitle>
        <DialogDescription id="incoming-call-description" className="sr-only">
          {callerName} sizga {callType === 'video' ? 'video ' : ''}qo‘ng‘iroq qilmoqda.
        </DialogDescription>
        ...
        <div className="relative flex flex-col items-center px-6 pb-6 pt-9">
          <div className="relative mb-5">
            <span
              className={`absolute -inset-3 rounded-full bg-white/10 ${isRinging ? 'animate-ping' : ''}`}
              style={{ animationDuration: '2s' }}
            />
            <span
              className={`absolute -inset-1.5 rounded-full border border-white/25 ${isRinging ? 'animate-pulse' : ''}`}
            />
          ...
          <p className="mt-1 text-center text-sm text-white/60">
            {callType === 'video' ? "sizga video qo'ng'iroq qilmoqda..." : "sizga qo'ng'iroq qilmoqda..."}
          </p>

          <div className="mt-8 flex items-start justify-center gap-1.5 sm:gap-3">
            <CallControlButton icon={Video} label="Video" tone="neutral" hasMenu onClick={handleAccept} />
            <CallControlButton icon={PhoneOff} label="Rad etish" tone="decline" onClick={handleDecline} />
            <CallControlButton
