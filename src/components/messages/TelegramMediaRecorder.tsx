...
export function TelegramMediaRecorder({ onSend, onCancel }: TelegramMediaRecorderProps) {
  ...
  if (state === 'recording' && recordingModeRef.current === 'video') {
    return (
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <Button variant="ghost" size="icon" onClick={cancelRecording} className="text-white hover:bg-white/10">
  ...
  if (state === 'recording' && recordingModeRef.current === 'voice') {
    return (
    ...
    <div className="flex items-center gap-1">
      <Button
        ...
        className={cn(
          ...
          mode === 'voice' && 'bg-accent'
        )}
        onClick={mode === 'voice' ? startRecording : toggleMode}
        title="Voice message"
      >
      ...
      <Button
        ...
        className={cn(
          'h-10 w-10 rounded-full transition-all',
          mode === 'video' && 'bg-accent'
        )}
        onClick={mode === 'video' ? startRecording : toggleMode}
        title="Video message"
      >
