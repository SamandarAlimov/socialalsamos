...
export function AIArtifactPanel({ artifacts, activeId, onSelect, onClose, isMobile }: AIArtifactPanelProps) {
  ...
  const active = useMemo(
    () => artifacts.find((artifact) => artifact.id === activeId) || artifacts[artifacts.length - 1],
    [activeId, artifacts],
  );
  ...
  return (
    <div
      className={cn(
        'flex h-full flex-col border-l border-border/50 bg-background',
    ...
    >
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-border/40 px-3 sm:h-14">
        ...
        {canPreview && (
          <Button size="sm" variant="ghost" className="h-8 gap-1.5 rounded-lg text-xs" onClick={() => setPreview((value) => !value)}>
            <Eye className="h-3.5 w-3.5" /> {preview ? 'Kod' : 'Ko‘rish'}
          </Button>
        )}
        {active && (
          <>
      ...
      <ScrollArea className="min-h-0 flex-1">
        {!active ? (
          <div className="flex min-h-[220px] items-center justify-center px-6 text-center text-xs text-muted-foreground">
            Hozircha artefakt yo‘q. Faqat alohida qayta ishlatiladigan kod, hujjat, rasm yoki video shu yerda ko‘rinadi.
          </div>
        ) : active.kind === 'image' ? (
          <div className="p-4">
            <img src={active.content} alt={active.title} className="w-full rounded-2xl border border-border/50 object-contain" />
          </div>
        ) : active.kind === 'video' ? (
          <div className="p-4">
            <video src={active.content} controls playsInline className="aspect-video w-full rounded-2xl bg-black object-contain" />
          </div>
        ) : canPreview && preview ? (
          <iframe title={active.title} sandbox="allow-scripts" srcDoc={active.content} className="h-full min-h-[420px] w-full border-0 bg-white" />
        ) : active.kind === 'code' ? (
          <pre className="m-0 max-w-full overflow-x-auto p-4 text-[12px] leading-relaxed"><code>{active.content}</code></pre>
        ) : (
          <div className="prose prose-sm max-w-none break-words p-4 dark:prose-invert [overflow-wrap:anywhere]">
        ...
        {run && (
          <div className="border-t border-border/40 p-3">
            ...
            {run.logs.length > 0 && <pre className="max-h-40 overflow-auto rounded-lg bg-muted/60 p-2 text-[11px]">{run.logs.join('\n')}</pre>}
            {run.result !== undefined && run.result !== null && (
              <pre className="mt-1 max-h-32 overflow-auto rounded-lg bg-muted/40 p-2 text-[11px]">{`→ ${JSON.stringify(run.result, null, 2)}`}</pre>
            )}
