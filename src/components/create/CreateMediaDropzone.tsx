...
export function CreateMediaDropzone({ onPick, className }: CreateMediaDropzoneProps) {
  return (
    <button
      ...
      onClick={onPick}
      className={cn(
        'group mx-4 mb-4 flex flex-col items-center justify-center gap-3 rounded-2xl',
        'border-2 border-dashed border-border/70 px-6 py-12 text-center transition',
        'hover:border-primary/60 hover:bg-muted/30 active:scale-[0.995] sm:mx-5',
        className,
      )}
    >
      <span className="flex h-16 w-16 items-center justify-center rounded-full bg-muted/60 transition group-hover:bg-primary/10">
      ...
      <span className="mt-1 inline-flex h-10 items-center rounded-full bg-primary px-5 text-[14px] font-medium text-primary-foreground">
        Qurilmadan tanlash
      </span>
    </button>
  );
