...
function isAllowedOrigin(origin: string) {
  ...
  try {
    ...
    if (
      url.protocol === 'https:' &&
      (host === 'alsamos.com' || host === 'www.alsamos.com' || host.endsWith('.alsamos.com'))
    ) {
      return true;
    }
...
export default async function handler(req: any, res: any) {
  ...
  if (!isAllowedOrigin(origin)) {
    res.status(403).json({ error: 'ORIGIN_NOT_ALLOWED', message: 'Hisobni o‘chirish so‘rovi rad etildi.' });
    return;
  }

  const authorization = header(req, 'authorization');
  if (!/^Bearer\s+\S+$/i.test(authorization)) {
    res.status(401).json({ error: 'AUTH_REQUIRED', message: 'Hisobni o‘chirish uchun qaytadan kiring.' });
    return;
  }

  const body = req.body && typeof req.body === 'object' ? req.body : null;
  if (!body || body.confirm !== 'DELETE') {
    res.status(400).json({ error: 'CONFIRMATION_REQUIRED', message: 'Hisobni o‘chirish tasdiqlanmadi.' });
    return;
  }
  ...
  try {
    const upstream = await fetch(`${url}/functions/v1/account-delete`, {
      method: 'POST',
      headers,
    ...
    try {
      ...
      payload = { error: 'UPSTREAM_INVALID_RESPONSE', message: 'Hisobni o‘chirish xizmati noto‘g‘ri javob qaytardi.' };
    }

    res.status(upstream.status).json(payload ?? {});
  } catch (error) {
    const details = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
    console.error('[account-delete-proxy] upstream fetch failed:', details.slice(0, 300));
    res.status(503).json({
      error: 'DELETE_SERVICE_UNAVAILABLE',
      message: 'Hisobni o‘chirish xizmati bilan aloqa o‘rnatilmadi. Qaytadan urinib ko‘ring.',
    });
  }
}
